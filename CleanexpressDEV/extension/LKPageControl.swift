...
//  CleanexpressDEV
//
//  Created by mohammed ichou on 21/06/2023.
//
...
class LKPageControl: UIControl {
  ...
  private func setupViews() {
    ...
    self.numberOfDots.forEach { dot in
    dot.layer.cornerRadius = dot.bounds.height/100
      self.addConstraints([
        
        dot.centerYAnchor.constraint(equalTo: self.stackView.centerYAnchor),
        dot.widthAnchor.constraint(equalTo: self.stackView.heightAnchor, multiplier: 0.55, constant: 0),
        dot.heightAnchor.constraint(equalTo: self.stackView.heightAnchor, multiplier: 0.25, constant: 0)
    ...
    _ = numberOfDots.map { (dot) in
      ...
      if dot.tag == selectedDot.tag {
        ...
        UIView.animate(withDuration: 0.2, animations: {
          ...
          dot.transform = CGAffineTransform.init(scaleX: 3, y: 1)
